 ...
 * 用于对搜狐网站上的新闻进行抓取
 */
public class SohuNewsParser extends AbstractNewsParser {
    ...
    protected String getContent(NodeFilter contentFilter, Parser parser) throws ParserException {
        ...
        if (content != null) {
            ...
            NodeFilter inFilter = new AndFilter(new TagNameFilter("div"), new HasAttributeFilter("itemprop", "articleBody"));
            NodeList inList = parser.extractAllNodesThatMatch(inFilter);
            StringBuilder inBuilder = new StringBuilder();
            for (int i = 0; i < inList.size(); i++) {
                Div inContenTag = (Div) inList.elementAt(i);
                inBuilder = inBuilder.append(inContenTag.getStringText());
            }
            if(inList.size() > 0) {
            	content = inBuilder.toString();
            }
	...
	protected String getSource(NodeFilter sourceFilter, Parser parser) throws ParserException {
        ...
        for (int i = 0; i < sourceList.size(); i++) {
            ...
            SimpleNodeIterator eles = sourceTag.elements();
            while(eles.hasMoreNodes()) {
            	Node node = eles.nextNode();
	...
	protected NewsNodeFilters getNodeFilters() {
        ...
        NodeFilter imgFilter = new AndFilter(new TagNameFilter("div"), new HasAttributeFilter("id", "contentText"));
        NodeFilter sourceFilter = new AndFilter(new TagNameFilter("span"), new HasAttributeFilter("class", "sc"));
